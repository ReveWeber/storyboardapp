        ...
        reset: function () {
            ...
            for (key in menu.items) {
                ...
                if (!this.$menuHolder.children('.wPaint-menu-icon-name-' + key).length) {
                    ...
                    // *** ADDED BY ME *** //
                    // remove duplicate pathname if necessary
                    if (menu.items[key].img.lastIndexOf(_this.wPaint.options.path) !== 0) {
                        var pathTrimOffset = _this.wPaint.options.path.length;
                        menu.items[key].img = menu.items[key].img.substring(pathTrimOffset);
                    }
